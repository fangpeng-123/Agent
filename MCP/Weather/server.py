...
def get_weather_forecast(location: str, days: int = 3) -> str:
    ...
    Args:
        location: 城市名称或LocationID，如"北京"或"101010100"
        days: 预报天数，可选 3、7、10、15、30，默认3天

    Returns:
        天气预报信息
    """
    return _get_weather_forecast(location, days)


def _get_hourly_forecast(location: str, hours: int = 24) -> str:
    """逐小时预报：获取指定城市的逐小时天气预报（业务逻辑实现）"""
    if not HEFENG_KEY:
        return "错误：未配置和风天气KEY密钥"
    ...
    try:
        ...
        if data.get("code") == "200":
            ...
            for i, hour in enumerate(hourly[:12]):  # 只显示前12小时
                output += (
...
def get_hourly_forecast(location: str, hours: int = 24) -> str:
    """
    逐小时预报：获取指定城市的逐小时天气预报

    Args:
        location: 城市名称或LocationID，如"北京"或"101010100"
        hours: 预报小时数，可选 24、72，默认24小时
...
def _get_air_quality(location: str) -> str:
    """空气质量：获取指定城市的实时空气质量（业务逻辑实现）"""
    if not HEFENG_KEY:
        return "错误：未配置和风天气KEY密钥"
...
def get_air_quality(location: str) -> str:
    ...
    Returns:
        空气质量信息
    """
    return _get_air_quality(location)


def _get_life_index(location: str) -> str:
    """生活指数：获取指定城市的生活指数建议（业务逻辑实现）"""
    if not HEFENG_KEY:
        return "错误：未配置和风天气KEY密钥"
    ...
    params = {
        ...
        "key": HEFENG_KEY,
        "type": "1,2,3,5,6,8,9",  # 运动、洗车、穿衣、紫外线、旅游、舒适度、感冒
    }
...
@mcp.tool()
def get_life_index(location: str) -> str:
    """
    生活指数：获取指定城市的生活指数建议
    ...
    Returns:
        生活指数信息
    """
    return _get_life_index(location)
...
@mcp.tool()
def search_city(city_name: str, country: str = "CN") -> str:
    """
    城市搜索：搜索城市信息
    ...
    Returns:
        搜索结果
    """
    return _search_city(city_name, country)
